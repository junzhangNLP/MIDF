...
def replace_mask_with_list(sentence, replacement_list,model_path):
    replaced_sentence = sentence
    if model_path == "../roberta-base":
        for replacement in replacement_list:
            # 替换第一个出现的[mask]
            replaced_sentence = replaced_sentence.replace("<mask>", replacement, 1)
    else:
        for replacement in replacement_list:
            # 替换第一个出现的[mask]
            replaced_sentence = replaced_sentence.replace("[MASK]", replacement, 1)
...
def collate_fn(data):
    ...
    labels_sent = [i['label'] for i in data]
    clear_sents = [i["text_a"] for i in data]
    ...
    return input_ids, att_post, attention_mask, alabels,labels_sent,clear_sents
...
def train_epoch(model, optimizer, train_loader, lossFunc, use_cuda):
    ...
    tot_loss = 0
    tot_len = 0
    for index, (input_ids, att_post, attention_mask, alabels,labels_sent,clear_sents) in enumerate(tqdm(train_loader,desc=f"Training...")):
...
def dev_evaluate(model, loader, lossFunc, use_cuda):
    ...
    with torch.no_grad():
        j = 0
        tot_len = 0
        for index, (input_ids, att_post, attention_mask, alabels,labels_sent,clear_sents) in enumerate(tqdm(loader, desc=f"Deving...")):
            if use_cuda:
                input_ids = input_ids.cuda()
                att_post = att_post.cuda()
                attention_mask = attention_mask.cuda()
...
def test_evaluate(model, loader, lossFunc, use_cuda, p, model_path):
    model.eval()
    with torch.no_grad():
        ...
        tot_len = 0
        data_list = []
        for index, (input_ids, att_post, attention_mask, alabels,labels_sent,clear_sents) in enumerate(tqdm(loader, desc=f"Testing...")):
            if use_cuda:
                input_ids = input_ids.cuda()
                att_post = att_post.cuda()
                attention_mask = attention_mask.cuda()
            ...
            for prediction, alabel, mask_pos in zip(predictions, alabels, attention_mask):
                ...
                for i in range(len(alabel)):
                    ...
                    if len(label_index) > 0:
                        ...
                        # 统计正确预测数量
                        if pred_values[label_index] > p:
                            ture_mask.append(tokenizer.decode(pred_indices[label_index]))
                        else:
                            ture_mask.append(tokenizer.decode(label))
                        allpresion += torch.sum(pred_values > p).item() if pred_values[label_index] <= p else 0
                ture_mask_list.append(ture_mask)
            for sent, mask_a,label_se in zip(clear_sents,ture_mask_list,labels_sent):
                data_list.append((replace_mask_with_list(sent,mask_a,model_path),label_se))
            total_samples += len(input_ids)
        accuracy = correct_predictions / tot_len
        precision = answer_predictions / allpresion
        
        return accuracy, precision,data_list

def train_and_evaluate(model, tokenizer, train_loader, dev_loader, test_loader, lossFunc, optimizer, num_epochs=20, p_list=[4, 4.5, 5, 5.5, 6], use_cuda=True, batch_size=8, learning_rate=1e-5, ways="Dataset_ipa", language="english", data_name="SST-2", sim=0.4, w_the=3, expock = "cover",model_path = ""):
    histary = []
    dev_list = []
    if use_cuda:
        model = model.cuda()
    ...
    result_df.to_csv(f'./results_EPLM_cover/{language}/{data_name}/train_{model_name}_{data_name}_{p_list}_sim={sim}_w_the={w_the}_expock={expock}.txt', sep='|', encoding='UTF-8', index=False)

    result_df = pd.DataFrame(dev_list, columns=['epoch', 'dev_acc'])
    result_df.to_csv(f'./results_EPLM_cover/{language}/{data_name}/dev_{model_name}_{data_name}_{p_list}_sim={sim}_w_the={w_the}_expock={expock}.txt', sep='|', encoding='UTF-8', index=False)
    ...
    with torch.no_grad():
        for p in p_list:
            acc, precision,data_list = test_evaluate(model, test_loader, lossFunc, use_cuda, p, model_path=model_path)
            acc_list.append([p, acc, precision])

    result_df = pd.DataFrame(data_list, columns=['text', 'label'])
    result_df.to_csv(f'./results_EPLM_cover/{language}/{data_name}/test_{model_name}_{expock}.txt', sep='\t', encoding='UTF-8', index=False)

    result_df = pd.DataFrame(acc_list, columns=['p', 'Acc', 'precision'])
    result_df.to_csv(f'./results_EPLM_cover/{language}/{data_name}/test_{model_name}_{data_name}_{p_list}_sim={sim}_w_the={w_the}_expock={expock}.txt', sep='|', encoding='UTF-8', index=False)
...
if model_path == "../roberta-base":
    for class_name in data_class:
        data = pd.read_csv(f"./dataset/{ways}/{language}/{data_name}_{class_name}_{sim}_{w_the}.txt", sep='|')
...
# 训练和评估
train_and_evaluate(model, tokenizer, train_loader, dev_loader, test_loader, lossFunc, optimizer, num_epochs, p_list, use_cuda=True, batch_size=batch_size, learning_rate=learning_rate, ways=ways, language=language, data_name=data_name, sim=sim, w_the=w_the, model_path = model_path)